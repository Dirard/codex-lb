...
- Upstream Responses WebSockets use transport ping/pong control frames to detect a black-holed connection without confusing valid application-event silence with an idle turn. Routed connections send proactive heartbeats at the smaller of 30 seconds and half `proxy_downstream_websocket_idle_timeout_seconds`; direct connections keep the transport library's native proactive cadence. The configured idle timeout remains the zero-config liveness budget.
